...
fn main() {
    ...
    let gl = unsafe { glow::Context::from_loader_function(|s| video_subsystem.gl_get_proc_address(s).unwrap() as *const _) };
    ...
    'running: loop {
        ...
        fn map_mouse_button(sdl_mb: sdl3::mouse::MouseButton) -> microui::MouseButton {
            match sdl_mb {
                sdl3::mouse::MouseButton::Left => microui::MouseButton::LEFT,
                sdl3::mouse::MouseButton::Right => microui::MouseButton::RIGHT,
                sdl3::mouse::MouseButton::Middle => microui::MouseButton::MIDDLE,
                _ => microui::MouseButton::NONE,
            }
        }

        fn map_keymode(sdl_km: sdl3::keyboard::Mod, sdl_kc: Option<sdl3::keyboard::Keycode>) -> microui::KeyMode {
            match (sdl_km, sdl_kc) {
                (sdl3::keyboard::Mod::LALTMOD, _) | (sdl3::keyboard::Mod::RALTMOD, _) => microui::KeyMode::ALT,
                (sdl3::keyboard::Mod::LCTRLMOD, _) | (sdl3::keyboard::Mod::RCTRLMOD, _) => microui::KeyMode::CTRL,
                (sdl3::keyboard::Mod::LSHIFTMOD, _) | (sdl3::keyboard::Mod::RSHIFTMOD, _) => microui::KeyMode::SHIFT,
                (_, Some(sdl3::keyboard::Keycode::Backspace)) => microui::KeyMode::BACKSPACE,
                (_, Some(sdl3::keyboard::Keycode::Return)) => microui::KeyMode::RETURN,
        ...
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::MouseMotion { x, y, .. } => state.ctx.input_mousemove(x as i32, y as i32),
                Event::MouseWheel { y, .. } => state.ctx.input_scroll(0, y as i32 * -30),
                Event::MouseButtonDown { x, y, mouse_btn, .. } => {
                    let mb = map_mouse_button(mouse_btn);
                    state.ctx.input_mousedown(x as i32, y as i32, mb);
                }
                Event::MouseButtonUp { x, y, mouse_btn, .. } => {
                    let mb = map_mouse_button(mouse_btn);
                    state.ctx.input_mouseup(x as i32, y as i32, mb);
                }
